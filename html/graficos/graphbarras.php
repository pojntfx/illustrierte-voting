...
if ($nvars > 0 && $nvars <= 50) {
	...
	for ($semestre = 0; $semestre < $nvars; $semestre++) {
		$percentage = (($valores[$semestre])*180)/max($valores);

		$puntos = array();

		// lado DERECHO DE LOS CUBOS (oscura)
		$puntos = CargaEsquinas($equis2 + 1, 49 + (180 - $percentage), $equis2 + 1, 229, $equis2 + 9, 220, $equis2 + 9, 41 + (180 - $percentage));

		imagefilledpolygon($image, $puntos, 4, $colores[$idcolor + 1]);


		// TOPE DE LOS CUBOS 3D (clara)
		$puntos = CargaEsquinas($equis1 + 9, 41 + (180 - $percentage), $equis1+1, 49 + (180 - $percentage), $equis2+1, 49 + (180 - $percentage), $equis2 + 9, 41 + (180 - $percentage));
		imagefilledpolygon($image, $puntos, 4, $colores[$idcolor + 2]);

		ImageFilledRectangle($image,$equis1 + 1,50 + (180 - $percentage),$equis2,229,$colores[$idcolor]);   // crea rectangulo

		//ImageString($image,1,$equis1 + 2, 232,$semestre,$colores[0]);
		ImageString($image,1,$equis1 + 2, 51 + (180 - $percentage),$valores[$semestre],$colores[4]);
